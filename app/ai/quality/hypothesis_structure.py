...
def classify_level(statement: str) -> str:
    ...
    if any(word in statement for word in result_words) and any(
        word in statement for word in causal_verbs
    ):
        return "财务结果维度"
    if any(word in statement for word in _LEVELS["需求/行业维度"]) and not any(
        word in statement for word in ("销量", "订单", "交付", "出口")
    ):
        return "需求/行业维度"
    scores = {level: sum(word in statement for word in cues) for level, cues in _LEVELS.items()}
    return (
        max(scores, key=lambda level: scores[level])
    ...
    for item in hypotheses:
        ...
        duplicate = next(
            (
                ...
                if tokens and prior and len(tokens & prior) / len(tokens | prior) >= 0.6
            ),
            None,
        )
        ...
        item["quality_warning"] = (
            f"与 H{duplicate} 高度重叠，建议合并或改写为不同因果环节" if duplicate else ""
        )
        seen.append(tokens)
    return hypotheses
