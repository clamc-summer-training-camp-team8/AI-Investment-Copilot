...
def usage_from_payload(payload: dict[str, Any], settings: Settings) -> ModelCallUsage:
    ...
    cost = (
        Decimal(input_tokens) * settings.llm_input_cost_per_million
        + Decimal(output_tokens) * settings.llm_output_cost_per_million
    ) / Decimal(1_000_000)
    ...
    return ModelCallUsage(
        ...
        cost_amount=cost.quantize(Decimal('0.00000001')),
    )
