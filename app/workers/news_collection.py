    ...
    **details: object,
    ...
    if raw is not None:
        try:
            ...
            previous = {}
    queued_this_run = int(str(details.get("queued", 0) or 0))
    queued_today = int(str(previous.get("queued_today", 0) or 0))
    if status == "completed":
...
def _covered_securities(conf: Settings) -> list[Any]:
    ...
    with uow_scope() as uow:
        ...
        return [
            ...
            if str(security.security_id) in covered_ids
            and str(security.security_id) not in excluded_ids
        ]
