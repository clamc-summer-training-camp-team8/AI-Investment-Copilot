...
def get_logic_change_detail(
    ...
    for item in digest.hypothesis_impacts:
        ...
        related_metrics = []
        for metric_id in _object_list(item.get("related_metric_ids")):
            mapping = next(
                (
                    value
            ...
            LogicChangeHypothesisImpactOut(
                ...
                paths=[
                    LogicChangeCausalPathOut(
                        ...
                        evidence_ids=[
                            str(value) for value in _object_list(path.get("evidence_ids"))
                        ],
                    )
                    for path in _object_list(item.get("paths"))
                    if isinstance(path, dict)
                ],
                related_metrics=list(dict.fromkeys(related_metrics)),
                evidence_ids=[str(value) for value in _object_list(item.get("evidence_ids"))],
            )
        )
...
def list_research_updates(
    ...
    records = [
        ...
        if (
            ...
            or (
                ...
                and (
                    cutoff is None
                    or (item.disclosed_at is not None and item.disclosed_at.date() >= cutoff)
                )
            )
        )
    ...
    if business_day is not None:
        digests = {
            ...
            for digest in uow.logic_change_digests.list_for_business_day(business_date=business_day)
        }
        # 历史回溯只呈现当时已完成归并的公司级卡片，避免混入旧的原子候选。
        cards = [card for card in cards if (card.security_id, card.thesis_id) in digests]
...
def _source_documents(records, uow, key_citations: set[str]) -> list[LogicChangeSourceDocumentOut]:
    ...
    documents: list[LogicChangeSourceDocumentOut] = []
    for entry in grouped.values():
        ...
        documents.append(
            LogicChangeSourceDocumentOut(
                ...
                published_at=(
                    entry["published_at"] if isinstance(entry["published_at"], datetime) else None
                ),
                source_url=str(entry["source_url"]) if entry["source_url"] else None,
                facts=[LogicChangeSourceFactOut(**fact) for fact in facts.values()],
            )
        )
    return sorted(
        documents,
