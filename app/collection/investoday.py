...
class InvestodayReport:
    ...
    def as_text(self) -> str:
        sections = [
            ("标题", self.title), ("发布时间", self.published_at.isoformat()),
            ("机构", self.institution_name), ("作者", self.author),
            ("核心观点", self.keyword), ("研报内容", self.content),
...
class InvestodayNewsClient:
    ...
    async def fetch_latest(self, *, page_size: int, stock_code: str | None = None) -> list[InvestodayNews]:
        try:
...
def _parse_report(row: dict[str, Any]) -> InvestodayReport | None:
    ...
    return InvestodayReport(
        ...
        institution_name=_text(row.get("institutionName")), author=_text(row.get("author")),
        content=_text(row.get("content")), keyword=_text(row.get("keyword")),
    )
