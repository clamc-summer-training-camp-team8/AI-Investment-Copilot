...
export function StatusDecisionPage() {
  ...
  const decision = useMutation({
    mutationFn: () => {
      ...
      if (!reason.trim()) throw new Error(`${action}时必须填写负责人决策理由。`)
            ...
            {suggestion ? (
              <div className="decision-panel">
                ...
                <div className="decision-form">
                  ...
                  <label>
                    <span>负责人决策理由 · 必填</span>
                    <textarea value={reason} onChange={(event) => setReason(event.target.value)} placeholder="请填写接受、拒绝或修改建议的依据" />
                  </label>
                  {error && <span className="form-error" role="alert">{error}</span>}
                  <button className="button primary wide" disabled={!action || !reason.trim() || (action === '修改' && !targetStatus) || decision.isPending} onClick={() => decision.mutate()}>提交人工决策并生成版本<Icon name="arrow" /></button>
                </div>
              </div>
            ) : (
