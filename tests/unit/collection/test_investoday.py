...
async def test_新闻客户端解析供应商分页结果(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    news = await InvestodayNewsClient(api_key="secret", base_url="https://example.test").fetch_latest(
        page_size=10
    )
...
def test_仅路由唯一命中的覆盖公司() -> None:
    ...
    assert not _item_mentions_security(InvestodayNews("N-3", "海外宏观市场新闻", datetime.now(tz=UTC)), securities[0])
