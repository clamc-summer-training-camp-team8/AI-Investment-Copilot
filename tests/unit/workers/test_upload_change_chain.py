...
def test_graph_rag_默认用_evidence_fusion_包装现有_runtime_retriever() -> None:
    ...
    segments = [
        DocumentSegmentRecord(
            ...
            locator="DOC-GRAPH#paragraph-1",
            ordinal=1,
            content="公司披露新签订单同比增长35%。",
        )
    ]
    events = extract_events_from_segments(
        "DOC-GRAPH",
        "NEW001",
        [(segments[0].locator, segments[0].content)],
        disclosure_time=disclosed_at,
    )
    settings = Settings(_env_file=None, llm_provider="local")
    ...
    result = process_events(
        ...
        actor=Actor(user_id="researcher-1"),
        thresholds=settings.rules,
        current_event_segments=segments,
        document_id="DOC-GRAPH",
        rag_settings=settings,
    )
...
def test_graph_rag_为公司唯一逻辑记录图画像与排序审计() -> None:
    ...
    segments = [
        DocumentSegmentRecord(
            ...
            locator="DOC-RANK#paragraph-1",
            ordinal=1,
            content="公司披露新签订单金额增长，订单需求改善。",
        )
    ]
    events = extract_events_from_segments(
        "DOC-RANK",
...
def test_worker_analyzes_each_hypothesis_in_company_thesis() -> None:
    uow = build_fake_uow()
    uow.securities.add(SecurityRecord(security_id="NEW001", name="新能源公司"))
    thesis_id = "THS-MULTI"
    uow.thesis.add(
    ...
    for index in (1, 2):
        uow.thesis.add_hypothesis(
            HypothesisRecord(
                ...
                thesis_id=thesis_id,
                statement=f"第{index}类新签订单增长支撑营业收入提升",
                hypothesis_type="经营",
                importance=Importance.CORE,
            )
        )
