...
class FakeRankingPriorRepo:
    ...
    def topics(self, *, security_id, direction, horizon):
        return [replace(row) for row in self.logic_topics.values()
                if row.security_id == security_id and row.direction == direction
                and row.horizon == horizon and row.status == "active"]

    def topic_relations(self, topic_id):
        return [replace(row) for row in self.logic_topic_relations.values()
    ...
    def ranked_items(self, snapshot_id, *, object_type, limit):
        rows = [replace(row) for row in self.items.values()
                if row.snapshot_id == snapshot_id and row.object_type == object_type
                and row.status == "active"]
