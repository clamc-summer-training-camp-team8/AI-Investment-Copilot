...
主要内容：
- 新增 `tests/fixtures/ai/evidence_cases.json`。
- 覆盖支持、冲突、正负线索并存的不确定、中性和资料不足场景。
- 新增固定样例回归测试，验证本地规则对事件影响方向的基本判断。

验证结果：AI 单元测试 13 passed。

后续规则词表、影响方向或证据评分逻辑变化时，必须同步维护固定样例和汇报文档。