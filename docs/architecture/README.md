...
- GraphRAG 的开发原理、实现落点、产品应用、效果演进、成熟度判断和后续路线见
  [`GraphRAG开发总结-20260901.md`](GraphRAG开发总结-20260901.md)。
...
单一事实来源。任何一处改动都是跨模块事件，需要 1 个 approve，并在 PR 里 @ 上受影响的消费方。
...
小改动（补充说明、修正笔误、更新目录树）直接提 PR，CI 绿了自己合。

改动分层依赖、模块边界或跨模块契约时：

1. 同步更新本文与 `layering.md`。改 `.importlinter` 或 `contracts/` 会触发 1 个 approve 的要求（这两类是全仓仅有的强制评审路径之一，见 [ADR-0005](../adr/0005-按团队规模裁剪评审强度.md)）。
2. 补一个 ADR 说明背景、方案、取舍、影响面。**ADR 不阻塞合并**，先合代码、一周内补即可。

放宽 `.importlinter` 里的约束时，PR 里要说明放宽后什么问题会不再被拦住。这是评审真正要看的东西。
