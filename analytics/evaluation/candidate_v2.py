...
_EXCLUSIONS = re.compile(
    ...
    # 港交所程式化披露。跨市场后这类标题占比很高（样本内 600 条），
    # 它们按期机械发布、不含经营信息，是误报的最大来源。
    # 「月报表截至31/7/2026」是证券变动月报，名字像销量月报，必须在主题判断前排除。
    r"|翌日披露报表|证券变动月报表|月报表截至|通知信函|公司通讯|董事名单与其角色"
    r"|暂停办理|过户登记|购股权|存续期|跟踪评级|受托管理事务"
)
...
_TOPIC_PATTERNS: tuple[tuple[str, re.Pattern[str]], ...] = (
    (
        "H1-需求与出货",
        re.compile(
            r"产销快报|产销数据|经营数据|交付数据|销量|出货|交付量|中标|签订|订单"
            r"|重大合同|框架协议|战略合作|采购|供货"
            # 医药：研发管线与注册上市决定「有没有东西可卖」，归需求侧。
            # 恒瑞样本内有 128 条临床试验批准公告，v2 原词表一条都抓不到。
            r"|临床试验|临床研究|突破性治疗|优先审评|快速通道|IND"
            r"|注册证书|注册批准|上市许可|获准上市|生产批件|补充申请"
        ),
    ),
    (
        "H2-盈利质量",
        re.compile(
            r"业绩预告|业绩快报|盈利警告|年度报告|半年度报告|季度报告|年报|中报"
            r"|业绩公布|业绩公告|中期业绩|减值|计提|毛利|营业收入"
            # 医药：集采与医保准入直接决定价格，是这个行业最典型的利润冲击事件
            r"|集中采购|带量采购|集采|医保目录|中选结果|谈判结果"
        ),
    ),
    (
        "H3-产能与扩张",
        re.compile(
            r"对外投资|投资设立|扩产|产能|建设项目|新建|增资|收购|合营公司"
            r"|募集资金|定向增发|可转债|发行.{0,6}股份|存托凭证|供股"
            # 半导体：制程与产线进展属于产能侧
            r"|晶圆|制程|流片|封测|良率|产线|先进封装"
        ),
    ),
)
