...
金融业务判断能力的导师确认。59 条独立盲标已经完成并单独保存在
`real_data/dataset/blind_annotation_result/`；本模块仍只负责可复算的预标注，不能因为
人工金标已存在就把程序生成标签改称金标。
...
# 兼容后端评测和历史实验的稳定导入名。业务含义仍由下方映射表统一维护。
H1 = "H1-需求与出货"
H2 = "H2-盈利质量"
H3 = "H3-产能与扩张"
...
def mentor_ruling(title: str) -> tuple[str, str] | None:
    """返回历史调用方需要的 ``(假设, 方向)``，规则编号仍由 ``apply_ruling`` 提供。"""
    ruling = apply_ruling(title)
    if ruling is None:
        return None
    _, hypothesis, direction = ruling
