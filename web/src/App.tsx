...
import { AssetPage, CompanyResearchPage, CoverageManagementPage, DocumentReaderPage, EvidencePage, LogicChangeImpactPage, MacroStrategyPage, NotFoundPage, OperationalWorkbenchPage, QualityPage, QuantPage, RadarPage, ResearchImpactDetailPage, ResearchUpdatesPage, RetrospectiveCenterPage, ReviewsPage, ThesisListPage, ThesisPage, WorkbenchPage } from './pages'
...
export function App() {
  ...
  return <div className={`app-shell ${isWorkbench ? 'workbench-shell' : ''} ${isCompanyResearch ? 'company-shell' : ''} ${isCoverageManagement ? 'coverage-shell' : ''} ${isMacroStrategy ? 'macro-shell' : ''} ${isResearchUpdates ? 'updates-shell' : ''} ${isRetrospective ? 'retrospective-shell' : ''} ${isLogicChange ? 'logic-change-shell' : ''} ${isDocumentReader ? 'document-reader-shell' : ''}`}>
    ...
    <div className="workspace"><header className="workspace-bar"><ResearchContextPicker theses={theses.data ?? []} value={currentThesisId} onChange={(id) => id ? navigate(`/radar?thesisId=${encodeURIComponent(id)}`) : navigate('/workbench')} /><div className="top-actions"><button className="button secondary" onClick={() => setShowUpload(true)}><Icon name="upload" size={15} />上传研究资料</button><button className="button primary" onClick={() => setShowCreate(true)}><span aria-hidden>＋</span>维护投资逻辑</button></div></header><main className="main-content" id="main-content" tabIndex={-1}><Routes><Route path="/" element={<Navigate to="/workbench" replace />} /><Route path="/workbench" element={<WorkbenchPage />} /><Route path="/operations" element={<OperationalWorkbenchPage />} /><Route path="/coverage" element={<CoverageManagementPage />} /><Route path="/macro-strategy" element={<MacroStrategyPage />} /><Route path="/updates" element={<ResearchUpdatesPage />} /><Route path="/updates/:updateId" element={<ResearchImpactDetailPage />} /><Route path="/logic-changes/:securityId/:thesisId" element={<LogicChangeImpactPage />} /><Route path="/documents/:documentId" element={<DocumentReaderPage />} /><Route path="/companies/geely" element={<CompanyResearchPage />} /><Route path="/radar" element={<RadarPage />} /><Route path="/radar/:evidenceId" element={<EvidencePage />} /><Route path="/theses" element={<ThesisListPage />} /><Route path="/theses/:thesisId" element={<ThesisPage />} /><Route path="/reviews" element={<ReviewsPage />} /><Route path="/retrospective" element={<RetrospectiveCenterPage />} /><Route path="/assets" element={<AssetPage />} /><Route path="/quality" element={<QualityPage />} /><Route path="/quant" element={<QuantPage />} /><Route path="*" element={<NotFoundPage />} /></Routes></main></div>
...
function CreateDraftDialog({ theses, onClose }: { theses: ThesisDetail[]; onClose: () => void }) {
  const navigate = useNavigate()
  const qc = useQueryClient()
  const securities = useQuery({ queryKey: ['securities'], queryFn: listSecurities })
  ...
  const existing = theses.find((item) => item.securityId === securityChoice)
  const mutation = useMutation({ mutationFn: async () => { if (existing) return existing; let securityId = securityChoice; if (securityChoice === '__new__') { if (!newSecurityId.trim() || !newSecurityName.trim()) throw new Error('新证券的代码和名称必填。'); const created = await createSecurity({ securityId: newSecurityId, name: newSecurityName, industry: newSecurityIndustry }); securityId = created.securityId; await qc.invalidateQueries({ queryKey: ['securities'] }) } if (!securityId) throw new Error('请选择或新建证券。'); return createDraft({ securityId, view, useRag }) }, onSuccess: async (thesis) => { await qc.invalidateQueries({ queryKey: ['theses'] }); onClose(); navigate(`/theses/${thesis.thesisId}`) } })
  return <div className="dialog-backdrop" role="presentation" onMouseDown={onClose}><section className="dialog create-dialog" role="dialog" aria-modal="true" onMouseDown={(event) => event.stopPropagation()}><span className="eyebrow">公司级研究主线</span><h2>维护投资逻辑</h2><p>每家公司只维护一条投资逻辑。已有逻辑通过版本化修订持续更新；新逻辑默认结合 Graph RAG 历史资料生成可证伪假设候选。</p><form className="form-grid" onSubmit={(event: FormEvent) => { event.preventDefault(); mutation.mutate() }}><label>投资对象<select value={securityChoice} onChange={(event) => setSecurityChoice(event.target.value)} required><option value="">选择已建档证券</option>{securities.data?.map((item) => <option key={item.securityId} value={item.securityId}>{item.securityId} · {item.name}{theses.some((thesis) => thesis.securityId === item.securityId) ? ' · 已有逻辑' : ''}</option>)}<option value="__new__">＋ 新建证券档案</option></select></label>{securityChoice === '__new__' && <div className="form-grid two nested-fields"><label>证券代码<input value={newSecurityId} onChange={(event) => setNewSecurityId(event.target.value)} required /></label><label>证券名称<input value={newSecurityName} onChange={(event) => setNewSecurityName(event.target.value)} required /></label><label>所属行业<input value={newSecurityIndustry} onChange={(event) => setNewSecurityIndustry(event.target.value)} /></label></div>}{existing ? <div className="success-note"><strong>{existing.title}</strong><br />该公司已有投资逻辑，将直接打开并通过修订维护，历史版本不会被覆盖。</div> : <><label>研究问题（可选）<textarea value={view} onChange={(event) => setView(event.target.value)} placeholder="留空则完全基于 RAG 历史资料生成" /></label><label className="checkbox-field"><input type="checkbox" checked={useRag} onChange={(event) => setUseRag(event.target.checked)} /> 使用权限过滤后的 Graph RAG 历史资料</label></>}<div className="dialog-actions"><button type="button" className="button secondary" onClick={onClose}>取消</button><button type="submit" className="button primary" disabled={mutation.isPending}>{mutation.isPending ? '处理中…' : existing ? '打开现有逻辑' : '生成草稿'}</button></div><InlineError error={mutation.error ?? securities.error} /></form></section></div>
}
