...
- 不要提交非公开信息、带授权限制的内容（付费数据库导出、研报原文）、个人信息、密钥、`.env`。
  演示样例数据仅限 `docs/data/数据分析交付包/业务样例包/`，且带 `is_illustrative` 标记。
  **公开披露数据例外**：`real_data/` 下的公告清单、定期报告财务数据与公开行情已纳入
  版本控制（[ADR-0006](docs/adr/0006-公开披露数据纳入版本控制.md)）。判断标准是
  「能否从官方公开渠道免费取得且无使用限制」。数据更新单独提交，别和代码混在一起。
