...
# Local development services must never be sent through a corporate/system
# HTTP proxy.  Without an explicit bypass, PowerShell and botocore can return a
# proxy-generated 502 even while PostgreSQL, MinIO and the API are healthy on
...
$env:NO_PROXY = (@($ExistingNoProxy + @("127.0.0.1", "localhost", "::1")) |
...
if ($PortsReachable) {
    ...
    # Feed the probe through stdin. Windows PowerShell/native argument parsing can
    # otherwise strip the nested quotes from `python -c` and turn a valid probe
    # into a misleading SyntaxError before startup continues.
    $PgvectorProbe = @'
import psycopg
...
raise SystemExit(0 if row and row[0] == "0.8.6" else 1)
'@
    $PgvectorProbe | & $Python -
