...
def main() -> None:
    ...
    parser.add_argument("--gold", type=Path, default=Path("analytics/evaluation/nine_company_blind_retrieval_gold_v1.json"))
    ...
    with session_scope() as session:
        ...
        docs = session.scalars(
            select(Document).where(Document.document_id.in_({doc_id for ids in title_to_ids.values() for doc_id in ids}))
        ).all()
        for doc in docs:
            document_meta[doc.document_id] = {"title": doc.title, "published_at": doc.published_at}
        valid_locators = set(session.execute(select(DocumentSegment.document_id, DocumentSegment.locator)).all())
    ...
    with uow_scope() as uow:
        for case in gold["cases"]:
            expected = set().union(*(title_to_ids.get(_canonical_title(title), set()) for title in case["expected_document_titles"]))
            snapshot_id, ranked = ranked_retrieval.search(
                uow,
                query=RankingQuery(text=case["query"], security_ids=(case["security_id"],), as_of=as_of, profile="primary_context", top_k=args.top_k),
                actor=actor,
                settings=settings,
            )
            hits = uow.assets.hybrid_search_segments(
                query=case["query"], query_embedding=embed_text(case["query"], version=settings.embedding_version or ""),
                embedding_version=settings.embedding_version or "", visibility_labels=tuple(sorted(actor.document_labels)),
                security_ids=(case["security_id"],), industries=(), published_from=None, published_to=as_of,
                keyword_weight=profile.keyword_weight, vector_weight=profile.vector_weight, limit=max(args.top_k * 5, 25),
            )
            baseline = rank_candidates(hits, priors={}, profile=profile, top_k=args.top_k, query_text=case["query"])
            ranked_ids = [item.document_id for item in ranked]
            baseline_ids = [item.document_id for item in baseline]
            future = [doc_id for doc_id in ranked_ids if document_meta.get(doc_id, {}).get("published_at") and document_meta[doc_id]["published_at"] > as_of]
            invalid_citations = [item.locator for item in ranked if (item.document_id, item.locator) not in valid_locators]
            rows.append({**case, "expected_document_ids": sorted(expected), "snapshot_id": snapshot_id,
                         "prior": _metrics(ranked_ids, expected, args.top_k), "baseline": _metrics(baseline_ids, expected, args.top_k),
                         "future_leakage_count": len(future), "invalid_citation_count": len(invalid_citations),
                         "returned_document_ids": ranked_ids})
    def avg(path: str) -> float:
        return round(sum(row[path.split('.')[0]][path.split('.')[1]] for row in rows) / len(rows), 4)
    summary = {
        "cases": len(rows), "top_k": args.top_k,
        ...
        "missing_gold_titles": sorted(title for title in titles if _canonical_title(title) not in title_to_ids),
    }
    summary["gate"] = {
    ...
    report = {"gold_version": gold["version"], "annotation_status": gold["annotation_status"], "as_of": as_of.isoformat(), "summary": summary, "results": rows}
